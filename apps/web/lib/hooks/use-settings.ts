...
export function useUpdateSocialLinks() {
  return useSWRMutation(
    `${API_URL}/api/settings/social-links`,
    (_, { arg }: { arg: { github?: string; twitter?: string; linkedin?: string; custom?: string[] } }) => api.settings.updateSocialLinks(arg)
  );
}

export function useUpdateAvatar() {
  return useSWRMutation(`${API_URL}/api/settings/avatar`, (_, { arg }: { arg: File }) => api.settings.updateAvatar(arg));
}

export function useUpdateEmail() {
  return useSWRMutation(`${API_URL}/api/settings/email`, (_, { arg }: { arg: { email: string } }) => api.settings.updateEmail(arg));
}

export function useUpdatePassword() {
  return useSWRMutation(`${API_URL}/api/settings/password`, (_, { arg }: { arg: { currentPassword: string; newPassword: string } }) =>
    api.settings.updatePassword(arg)
  );
}

export function useDeleteAccount() {
  return useSWRMutation(`${API_URL}/api/settings/account`, () => api.settings.deleteAccount());
}
